...
export default function Login({ setIsLogined, navigate }) {
    ...
    function handleSubmit(event) {
        ...
        if (email !== '' && password !== '') {
            ...
            const url = DOMAIN_API + "admins-login";
    ...
    if (localStorage.getItem('access_token')) {
        ...
        const role = localStorage.getItem('check_role');
        setIsLogined(true);
        if(role == 1){
            ...
            <Grid container spacing={2}>
                <Grid item xs={6}>
                    <div style={{ margin: "60px", justifyContent: "center" }}>
                        <form>
                            <h2>Đăng nhập Admin KOLsMarketing</h2>
                            <span style={{
                                fontWeight: 'bold',
                                color: 'red',
                            }}>{canLogin}</span>
