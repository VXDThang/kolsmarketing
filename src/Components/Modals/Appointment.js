...
export default function Appointment(props) {
    ...
    const { isOpen, isClose, selectedImage, selectedVideo, content, selectedPage } = props;
    ...
    async function handleSave(event) {
        ...
        //Vì getMonth lấy giá trị từ 0 ->11
        ...
        //+ ngày hiện tại lên 1
        let day_draft = addDays(today_new,1)
        let day_new_plus1Hour = day_draft.getDate() < 10 ? '0' + day_draft.getDate() : day_draft.getDate();
        let month_new_plus1Hour = (day_draft.getMonth() + 1) < 10 ? '0' + (day_draft.getMonth() + 1) : (day_draft.getMonth() + 1);
        ...
        if ((today_new.getHours() + 1) <= 23) {
            ...
            dateCurent_new = day_draft.getFullYear() + '-' + month_new_plus1Hour + '-' + day_new_plus1Hour;
        }
        ...
        if (temp1 > temp2) {
            ...
            if (props.selectedImage !== null) {
                ...
                if (json) {
                    fetch(DOMAIN_API + `social/post-social-schedule`, {
                        method: "POST",
                        headers: {
                            'Content-Type': 'application/json',
                            "x-access-token": actoken
                        },
                        body: JSON.stringify({ postText: content, id_page_social: selectedPage.id_page_social, id_page: selectedPage.id, image_url: json.secure_url, video_url: null, time: temp1.getTime() / 1000, id_job: null })
                    })
                        .then(res => res.json())
                        .then(
                            (result) => {
                ...
                if (props.selectedVideo !== null) {
                    ...
                    if (json) {
                        fetch(DOMAIN_API + `social/post-social-schedule`, {
                            ...
                            body: JSON.stringify({ postText: content, id_page_social: selectedPage.id_page_social, id_page: selectedPage.id, video_url: json.secure_url, image_url: null, time: temp1.getTime() / 1000, id_job: null })
                        })
                            .then(res => res.json())
                            .then(
                                (result) => {
                    ...
                    fetch(DOMAIN_API + `social/post-social-schedule`, {
                        ...
                        body: JSON.stringify({ postText: content, id_page_social: selectedPage.id_page_social, id_page: selectedPage.id, video_url: null, image_url: null, time: temp1.getTime() / 1000, id_job: null })
                    })
                        .then(res => res.json())
                        .then(
                            (result) => {
    ...
    return (
        <div>
            ...
            <Dialog
                ...
                aria-describedby=""
            >
                <DialogTitle>{"Đã lưu thành công. Nhấn Tắt để thoát"}</DialogTitle>
                <DialogActions>
                    <Button onClick={handleCloseAll}>Tắt</Button>
                </DialogActions>
            </Dialog>
