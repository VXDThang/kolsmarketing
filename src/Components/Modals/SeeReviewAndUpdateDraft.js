...
export default function SeeReviewAndUpdateDraft(props) {
    const { isOpen, isClose, id_job, id_post, review_detail, loadingToParent } = props;
    //copy from Job_Post_Write.js
    let actoken = localStorage.access_token;
    const [loading, setLoading] = React.useState(false);
    const [content, setContent] = React.useState(review_detail?.content);
    const [selectedImage, setSelectedImage] = React.useState(null);
    const [imageShow, setImageShow] = React.useState(review_detail?.url_image);
    const [changeSelectImageOrVideo, setChangeSelectImageOrVideo] = React.useState(false);
    const [selectedVideo, setSelectedVideo] = React.useState(null);
    const [videoShow, setVideoShow] = React.useState(review_detail?.url_video);
    ...
    const handleCloseModalNotUpdate = () => {
        loadingToParent(true);
        isClose(false);
    };
    ...
    async function handleSubmitDraftAgain(event) {
        setIsSaving(true)
        //Case có ảnh
        if (selectedImage !== null || imageShow !== null) {
          ...
          if (changeSelectImageOrVideo)  // Nếu đổi ảnh cũ
          {
            ...
            if (json) {
              fetch(DOMAIN_API + `social/update-draft-post`, {
                ...
                body: JSON.stringify({ postText: content, image_url: json.secure_url, video_url: null, id: review_detail.id })
              })
                .then(res => res.json())
                .then(
                  (result) => {
          ...
          else //Không thay đổi ảnh cũ
          {
            fetch(DOMAIN_API + `social/update-draft-post`, {
              ...
              body: JSON.stringify({ postText: content, image_url: imageShow, video_url: null, id: review_detail.id })
            })
              .then(res => res.json())
              .then(
                (result) => {
          ...
          //case có video
          if (selectedVideo !== null || videoShow !== null) {
            if (changeSelectImageOrVideo)  //Thay đổi video cũ
            {
              ...
              if (json) {
                fetch(DOMAIN_API + `social/update-draft-post`, {
                  ...
                  body: JSON.stringify({ postText: content, video_url: json.secure_url, image_url: null, id: review_detail.id })
                })
                  .then(res => res.json())
                  .then(
                    (result) => {
              ...
              else //Không thay đổi video cũ
              {
                fetch(DOMAIN_API + `social/update-draft-post`, {
                  ...
                  body: JSON.stringify({ postText: content, video_url: videoShow, image_url: null, id: review_detail.id })
                })
                  .then(res => res.json())
                  .then(
                    (result) => {
            ...
            fetch(DOMAIN_API + `social/update-draft-post`, {
              ...
              body: JSON.stringify({ postText: content, video_url: null, image_url: null, id: review_detail.id })
            })
              .then(res => res.json())
              .then(
                (result) => {
    ...
    React.useEffect(() => {
        setContent(review_detail?.content)
    }, [props])
                            ...
                            <Tooltip title="Thoát">
                                <Avatar sx={{ width: 28, height: 28, "&:hover": { bgcolor: "#DDDDDD" } }}>
                                    <CloseIcon sx={{ fontSize: 18 }}
                                        onClick={handleCloseModalNotUpdate} />
                                </Avatar>
                            </Tooltip>
                        </div>
                    </div>
                    <div style={{ paddingTop: "5px", paddingBottom: "30px", paddingLeft: "20px", paddingRight: "20px" }}>

                        <div>
                            <div style={{fontWeight:500, fontSize:"18px"}}>
                                Nhận xét từ nhãn hàng
                            </div>
                            <Divider sx={{ color: "#F0A318", paddingTop: "5px" }} />
                            <div  style={{fontWeight:500, paddingTop:"10px" }}>
                                Trạng thái
                            </div>
                            <div>
                                {review_detail?.type_accept == "0" ? <span>Chưa nhận xét</span> : ""}
                                {review_detail?.type_accept == "1" ? <span>Chấp nhận</span> : ""}
                                {review_detail?.type_accept == "2" ? <span>Cần chỉnh sửa</span> : ""}
                            </div>
                            <div style={{fontWeight:500, paddingTop:"10px" }}>
                                Lời nhận xét
                            </div>
                            <div>
                                {review_detail?.review ? review_detail.review : "Không có thông tin"}
                            </div>
                        </div>
                        ...
                        >
                            <DialogTitle>{"Cập nhật thành công"}</DialogTitle>
                            <DialogActions>
